...
const Roadmap = () => {
    ...
    setCompletedItems((prev) => {
      ...
      roadmapData.forEach((section) => {
        if (section.items.length > 0) {
          const allDone = section.items.every(
            (_, i) => updated[`${section.id}-${i}`]
          );
          if (allDone) {
  ...
  const getCompletionPercentage = () => {
    const total = roadmapData.reduce(
      (sum, section) => sum + Math.max(1, section.items.length || 1),
      0
    );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-black via-[#0f0f3f] to-black text-white pt-24 px-4 pb-4 sm:px-6 sm:pb-6">
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-10 sm:mb-12">
          <h1 className="text-3xl sm:text-5xl font-bold text-white mb-3 bg-gradient-to-r from-yellow-400 to-orange-500 bg-clip-text text-transparent">
            Java Buddy Roadmap
          </h1>
          <p className="text-base sm:text-lg text-gray-400 mb-4">
            Track your Java learning progress
          </p>
        ...
        <div className="relative">
          <div className="absolute left-1/2 transform -translate-x-1/2 w-1 bg-gradient-to-b from-purple-500 to-blue-500 h-full opacity-20 hidden sm:block" />
          ...
          {roadmapData.map((section, index) => (
            ...
            >
              <div className="sm:absolute sm:left-1/2 sm:transform sm:-translate-x-1/2 sm:-translate-y-1/2 sm:top-0 flex justify-center mb-4 sm:mb-0">
                <div
                  className={`w-12 h-12 sm:w-14 sm:h-14 flex items-center justify-center rounded-full shadow-xl border-4 border-white bg-gradient-to-br ${section.color}`}
                >
              ...
              >
                <div className="w-full sm:w-6/12 px-2 sm:px-4">
                  <div className="bg-white/10 backdrop-blur-md rounded-xl p-4 sm:p-6 border border-white/20 shadow-xl hover:bg-white/15 transition duration-300 hover:scale-[1.02]">
                    <div
        ...
        <div className="text-center mt-16">
          <div className="bg-white/5 p-6 sm:p-8 rounded-xl border border-white/10">
            ...
            <p className="text-gray-400 mb-4">
              Bookmark your progress and come back tomorrow!
            </p>
            <button
        ...
        {showReviewModal && (
          <div className="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm flex justify-center items-center z-50">
            <div className="bg-slate-300 text-black p-6 sm:p-8 rounded-lg w-11/12 sm:max-w-md shadow-2xl relative">
              <h3 className="text-lg sm:text-xl font-bold mb-4 text-center">
                Your Java Learning Summary
              </h3>
              <div className="space-y-2 max-h-64 overflow-y-auto">
                {roadmapData.map((section) => {
                  const subCount = section.items.length;
                  const subDone = section.items.filter(
                    (_, i) => completedItems[`${section.id}-${i}`]
                  ).length;
                  const total = subCount > 0 ? subCount : 1;
                  const completed =
                    subDone +
                    (completedItems[section.id] && subCount === 0 ? 1 : 0);
                  return (
                    <div
                      key={section.id}
